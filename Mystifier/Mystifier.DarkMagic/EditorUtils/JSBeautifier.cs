...
namespace Mystifier.DarkMagic.EditorUtils
{
    ...
    public class Beautifier
    {
        ...
        public Beautifier(JSBeautifier.BeautifierOptions opts)
        {
            Opts = opts;
            BlankState();
        }

        public JSBeautifier.BeautifierOptions Opts { get; set; }

        public JSBeautifier.BeautifierFlags Flags { get; set; }

        private List<JSBeautifier.BeautifierFlags> FlagStore { get; set; }
        ...
        private void BlankState()
        {
            // internal flags
            Flags = new JSBeautifier.BeautifierFlags("BLOCK");
            FlagStore = new List<JSBeautifier.BeautifierFlags>();
            WantedNewline = false;
        ...
        {
            var prev = new JSBeautifier.BeautifierFlags("BLOCK");
            if (Flags != null)
            {
                FlagStore.Add(Flags);
                prev = Flags;
            }
            Flags = new JSBeautifier.BeautifierFlags(mode);
        ...
        public string Beautify(string s, JSBeautifier.BeautifierOptions opts = null)
        {
            if (opts != null)
                Opts = opts;
        ...
        private Tuple<string, string> GetNextToken()
        {
            ...
            if (c == '/')
            {
                ...
                if (Input[ParserPos] == '*') // peek /* .. */ comment
                {
                    ...
                    {
                        while (
                            !(Input[ParserPos] == '*' && ParserPos + 1 < Input.Length && Input[ParserPos + 1] == '/') &&
            ...
            if (c == '\'' || c == '"' ||
                (c == '/' &&
                 ((LastType == "TK_WORD" && IsSpecialWord(LastText)) ||
                  (LastType == "TK_END_EXPR" &&
                   (Flags.PreviousMode == "(FOR-EXPRESSION)" || Flags.PreviousMode == "(COND-EXPRESSION)")) ||
                  new[]
                  {
                      "TK_COMMENT", "TK_START_EXPR", "TK_START_BLOCK", "TK_END_BLOCK", "TK_OPERATOR", "TK_EQUALS", "TK_EOF",
                        ...
                        while (esc || Input[ParserPos] != sep)
                        {
                            ...
                            if (esc1 != 0 && esc1 >= esc2)
                            {
                                if (
                                    !int.TryParse(
                                        new string(
                                            resultingString.Skip(Math.Max(0, resultingString.Count() - esc2))
                                                .Take(esc2)
                                                .ToArray()), NumberStyles.HexNumber, CultureInfo.CurrentCulture,
                                        out esc1))
                                    esc1 = 0;
        ...
        private void HandleStartBlock(string tokenText)
        {
            ...
            if (Opts.BraceStyle == JSBeautifier.BraceStyle.Expand)
            {
        ...
        private void HandleWord(string tokenText)
        {
            ...
            if (LastType == "TK_END_BLOCK")
            {
                ...
                {
                    if (Opts.BraceStyle == JSBeautifier.BraceStyle.Expand ||
                        Opts.BraceStyle == JSBeautifier.BraceStyle.EndExpand)
                        prefix = "NEWLINE";
                    else
                    {
                        prefix = "SPACE";
            ...
            if (tokenText == "else" || tokenText == "catch" || tokenText == "finally")
            {
                if (LastType != "TK_END_BLOCK" || Opts.BraceStyle == JSBeautifier.BraceStyle.Expand ||
                    Opts.BraceStyle == JSBeautifier.BraceStyle.EndExpand)
                    AppendNewline();
                else
                {
