...
namespace ExaPhaser.FilePicker
{
    /// <summary>
    ...
    public class FilePickerFragment : DialogFragment
    {
        ...
        private const string LogTag = "ExaPhaser.FilePicker.FileListFragment";
        ...
        public override void OnCreate(Bundle savedInstanceState)
        {
            ...
            if (Arguments == null)
            {
                throw new IllegalArgumentException(
                    "You must create DirectoryChooserFragment via the FileListFragment(string, string, FilePickerMode) constructor.");
            }
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            switch (filePickerMode)
            {
                case FilePickerMode.File:
                    view.FindViewById<TextView>(Resource.Id.txtvSelectedFolderLabel).Text =
                        GetString(Resource.String.filepicker_selected_file_label);
                    break;

                case FilePickerMode.Directory:
                    view.FindViewById<TextView>(Resource.Id.txtvSelectedFolderLabel).Text =
                        GetString(Resource.String.filepicker_selected_folder_label);
                    break;
        ...
        private void RefreshFilesList(File targetDirectory)
        {
            ...
            else
            {
                        ...
                        case FilePickerMode.Directory:
                                            ...
                                            && (item.Attributes & FileAttributes.Hidden) != FileAttributes.Hidden))
                            {
                                try
                                {
                                    ...
                                    //Throws an exception if it can't access the folder
                                    ...
                                    javaFile.Dispose();
                                    // remember to dispose to avoid keeping references to java objects.
                                }
                                catch (Exception ex)
                                {
                ...
                catch (Exception ex)
                {
                    ...
                    Toast.MakeText(Activity, "Problem retrieving contents of " + targetDirectory, ToastLength.Long)
                        .Show();
                    return;
                }
        ...
        private void CreateFileObserver(string path)
        {
            ...
            const FileObserverEvents Mask =
                FileObserverEvents.Create | FileObserverEvents.Delete | FileObserverEvents.MovedFrom |
                FileObserverEvents.MovedTo | (FileObserverEvents)0x40000000;
