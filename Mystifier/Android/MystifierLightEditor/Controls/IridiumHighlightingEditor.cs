...
namespace MystifierLightEditor.Controls
{
    ...
    public class IridiumHighlightingEditor : EditText
    {
        ...
        public RegexHighlightingDefinition HighlightingDefinition { get; set; }
...
#pragma warning restore RECS0130 // 'for' loop control variable is never modified
        ...
        private IEditable Highlight(IEditable e)
        {
            try
            {
                ...
                Matcher m = HighlightingDefinition.NumbersPattern.Matcher(e);
                for (int n = ErrorLine;
                n-- > 0 && m.Find();) ;

                e.SetSpan(
                    new BackgroundColorSpan(HighlightingDefinition.NumberColor),
                    m.Start(),
                    m.End(),
                    SpanTypes.ExclusiveExclusive);
            }
