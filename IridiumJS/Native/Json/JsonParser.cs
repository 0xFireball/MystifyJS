...
namespace IridiumJS.Native.Json
{
    public class JsonParser
    {
        ...
        public JsonParser(IridiumJSEngine engine)
        {
            _engine = engine;
        }
            ...
            for (var i = 0; i < len; ++i)
            {
                ...
                else
                {
                    throw new JavaScriptException(_engine.SyntaxError,
                        string.Format("Expected hexadecimal digit:{0}", _source));
                }
            }
            return (char) code;
        }
        ...
        private Token ScanStringLiteral()
        {
            ...
            while (_index < _length)
            {
                ...
                {
                    throw new JavaScriptException(_engine.SyntaxError,
                        string.Format("Invalid character '{0}', position:{1}, string:{2}", ch, _index, _source));
                }
                ...
                if (ch == '\\')
                {
                    ...
                    if (ch > 0 || !IsLineTerminator(ch))
                    {
                        switch (ch)
                        {
                            ...
                            case 'x':
                                var restore = _index;
                                var unescaped = ScanHexEscape(ch);
                                if (unescaped > 0)
                                {
        ...
        public ObjectInstance ParseJsonObject()
        {
            ...
            while (!Match("}"))
            {
                ...
                {
                    throw new JavaScriptException(_engine.SyntaxError,
                        string.Format("Invalid character in property name '{0}'", name));
                }
        ...
        public JsValue Parse(string code, ParserOptions options)
        {
            ...
            try
            {
                ...
                {
                    throw new JavaScriptException(_engine.SyntaxError,
                        string.Format("Unexpected {0} {1}", _lookahead.Type, _lookahead.Value));
                }
                return jsv;
            }
            finally
